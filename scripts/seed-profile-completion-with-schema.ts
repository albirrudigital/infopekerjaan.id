...
async function createSchema() {
  ...
  try {
    // Check if profile_completion_items table exists
    const tableExists = await db.execute(sql`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'profile_completion_items'
      );
    `);
    ...
    if (!exists) {
      console.log("Creating profile_completion_items table...");
      ...
      console.log("Creating user_profile_completions table...");
      await db.execute(sql`
        CREATE TABLE IF NOT EXISTS "user_profile_completions" (
          "id" SERIAL PRIMARY KEY,
